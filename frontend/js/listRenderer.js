// -------------------------------------------------------
// listRenderer.js – PDF listesini render etme
// -------------------------------------------------------
...
function renderList() {
  ...
  if (AppState.pdfFiles.length === 0) {
    $('pdfList').innerHTML = `
      <div class="empty-state">
        ...
        <h3>Henüz PDF yüklenmedi</h3>
        <p>PDF eklemek için "PDF Yükle" butonuna tıklayın veya dosyaları sürükleyin.</p>
      </div>`;
    updateInfoBar();
    return;
  }
    ...
    html += `
      <div class="pdf-item ${sel ? 'selected' : ''}" data-id="${f.id}">
        ...
        <span class="col-category">
          <span class="cat-badge ${!f.category ? 'none' : ''}" style="${f.category ? 'background:' + catColor : ''}">${f.category || '-'}</span>
        </span>
        <span class="col-actions">
          <button class="btn-icon info-icon" data-action="info" data-id="${f.id}" title="Bilgi">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>
          </button>
          <button class="btn-icon print-icon" data-action="print" data-id="${f.id}" title="Yazdır">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 01-2-2v-5a2 2 0 012-2h16a2 2 0 012 2v5a2 2 0 01-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg>
          </button>
          <button class="btn-icon" data-action="remove" data-id="${f.id}" title="Kaldır">
  ...
  $('pdfList').querySelectorAll('[data-action]').forEach(btn => {
    btn.addEventListener('click', function(e) {
      ...
      if (action === 'info') showInfoPanel(id);
      else if (action === 'print') printSingle(id);
      else if (action === 'remove') removeSingle(id);
    });
  });
